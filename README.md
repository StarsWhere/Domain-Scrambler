...
**定位**：一个 Python 域名变体生成工具，用于根据输入域名生成可能的混淆、仿冒或扰动候选。
...
**技术栈**：Python 标准库；建议通过 uv 运行。

**目录与模块**：`domain_scrambler.py` 是核心脚本，`LICENSE` 与 README 提供项目说明。

**使用方式**：使用 `uv run domain_scrambler.py` 执行，并按脚本参数传入目标域名。

**配置说明**：当前无独立配置文件，规则主要由脚本实现。

**适用场景**：适合钓鱼域名排查、品牌域名保护、威胁情报候选生成。

**注意事项**：仅用于授权安全研究和防护用途，不应用于生成误导性或欺诈性域名。
